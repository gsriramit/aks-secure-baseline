...
# assigning policies for an RBAC enabled key-vault does not work- so role assignment needs to be done
#az keyvault set-policy --certificate-permissions import get -n $KEYVAULT_NAME --object-id $SP_OBJECTID

# Role assignment needs to be done at the pre-deployment stage. The SP will be granted "Key Vault Certificates Officer" role at the subscription or a suitable RG level
